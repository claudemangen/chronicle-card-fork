...
export class HistoryAdapter implements ISourceAdapter {
  ...
  private stateChangeToEvent(
    ...
    // Resolve image_template, clip_url_template and action overrides (per-entity > source-level)
    const imageTemplate = entityConf?.image_template || this.config.image_template;
    const clipUrlTemplate = entityConf?.clip_url_template || this.config.clip_url_template;
    ...
    return {
      ...
      metadata: {
        ...
        _clip_url_template: clipUrlTemplate || undefined,
      },
    };
  }
}
